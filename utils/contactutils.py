...
def batch_mesh_contains_points(
    ...
    direction=torch.Tensor([0.4395064455, 0.617598629942,
                            0.652231566745]), device="cpu"
):
    """Time efficient but memory greedy !
    ...
    batch_direction = direction.view(1, 1, 3).expand(batch_size, triangle_nb,
                                                     3)

    # Compute ray/triangle intersections
    pvec = torch.cross(batch_direction, v0v2, dim=2)
    dets = torch.bmm(v0v1.view(batch_points_size, 1, 3),
                     pvec.view(batch_points_size, 3,
                               1)).view(batch_size, triangle_nb)
    ...
    ).view(batch_size, tvec.shape[1]) * invdet)
    ...
    ).view(batch_size, qvec.shape[1]) * invdet)
